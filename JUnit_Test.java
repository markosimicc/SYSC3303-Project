...
import scheduler_state.Scheduler.State;
...
class JUnit_Test {
	...
	@Test
	public void Test8() {
		assertEquals(sch.getState(sch.st),State.WAITING);
		sch.st = sch.st.next(trans_List.RECEIVE);
		assertEquals(sch.getState(sch.st),State.SCHEDULING);
		sch.st = sch.st.next(trans_List.SEND);
		assertEquals(sch.getState(sch.st),State.WAITING);
	}
	...
	public void Test9() {
		sch.st = sch.st.next(trans_List.SEND);
		assertEquals(sch.getState(sch.st),State.ILLEGAL);
	}
