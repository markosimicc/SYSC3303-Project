...
import java.time.*;
/*
 * Junit Testing for Elevator Simulator including testing of the states, and testing different parts of the system
 * 
 * @author Marko Simic, Mahmoud Alsous, Ross Matthew
 */
...
class JUnit_Test {
	...
	ElevatorSubsystem elev = new ElevatorSubsystem(1,sch);
	int num = 1;	
	...
	@Test
	//Test FloorSubsystem's readInput method for one input (Last line)
	public void Test7() {
		...
		RequestInfo testInfo = new RequestInfo("Up",1,6, timeTest);
	...
	//test scheduler state switch from waiting to scheduling
	@Test
	public void Test8() {
		assertEquals(sch.getState(sch.st),Scheduler.State.WAITING);
		sch.st = sch.st.next(SchedulerTransistions.RECEIVE);
		assertEquals(sch.getState(sch.st),Scheduler.State.SCHEDULING);
		sch.st = sch.st.next(SchedulerTransistions.SEND);
		assertEquals(sch.getState(sch.st),Scheduler.State.WAITING);
	}
	//test invalid transitions for scheduler
	@Test
	public void Test9() {
		sch.st = sch.st.next(SchedulerTransistions.SEND);
		assertEquals(sch.getState(sch.st),Scheduler.State.ILLEGAL);
		sch.st = sch.st.next(SchedulerTransistions.RECEIVE);
		sch.st = sch.st.next(SchedulerTransistions.RECEIVE);
		assertEquals(sch.getState(sch.st),Scheduler.State.ILLEGAL);
		
	}
	//test Elevator state machine
		@Test
		public void Test10() {
			assertEquals(elev.stat,ElevatorSubsystem.ElevState.WAITING);
			elev.stat = elev.stat.next(ElevatorTransistions.Door_closing);
			assertEquals(elev.getState(elev.stat),ElevatorSubsystem.ElevState.Door_Closed);
			elev.stat = elev.stat.next(ElevatorTransistions.Start_Motor);
			assertEquals(elev.getState(elev.stat),ElevatorSubsystem.ElevState.Moving);
			elev.stat = elev.stat.next(ElevatorTransistions.Reach_Floor);
			assertEquals(elev.getState(elev.stat),ElevatorSubsystem.ElevState.Stopped);
			elev.stat = elev.stat.next(ElevatorTransistions.Door_Opening);
			assertEquals(elev.getState(elev.stat),ElevatorSubsystem.ElevState.WAITING);	
		}
		//test Elevator state machine for illegal transistions
		@Test
		public void Test11() {
			elev.stat = elev.stat.next(ElevatorTransistions.Door_Opening);
			assertEquals(elev.getState(elev.stat),ElevatorSubsystem.ElevState.ILLEGAL);
			elev.stat = elev.stat.next(ElevatorTransistions.Reach_Floor);
			assertEquals(elev.getState(elev.stat),ElevatorSubsystem.ElevState.ILLEGAL);
